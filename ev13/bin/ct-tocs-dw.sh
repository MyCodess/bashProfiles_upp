...
echo; echo "_____________ converting HTMLs to TEXTs : _______________________________________________"
for ii in *.html; do html2text  --ignore-emphasis --ignore-links  --ignore-images  --dash-unordered-list "${ii}" > "${ii}.txt"; done;

echo; echo "_____________ putting all DWs of the year into one big test file : ______________________"
for ii in *.txt; do echo "${q_fold1Sm} ${ii} ==================================="; cat "${ii}"; echo "${q_fold1Em}"; echo; done  >| ../CT-TOCs-${year11}.txt
