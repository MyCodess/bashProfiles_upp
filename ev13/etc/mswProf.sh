...
export  HISTFILE="${ettcUser}/.hs1_ev1"   ##--will be overwriteen in hostProf!  here as general-/default-setting for msw-syys !
...
##--??:  export  HOME="${HOMEW_cyg}"   #--!-if you want to reset the msys-home to msw-home as default! but for now not!
alias sethomew='export   HOME=$HOMEW'   ##== ${USERPROFILE}'   #-sets the HOME envar to msw-HOME-dir in msw-path-format !
alias sethomewcyg='export   HOME=$HOMEW_cyg'   #-sets the HOME envar to msw-HOME-dir but in Lx-path-format !
...
alias viwinsevvprofs="vi1 -p $q_mswps1_ProfileFP  ${q_evv_mswps1_ProfileFD}/*"
###________________________________________  ___________________________
...
#-OS-apps:
chrom1_mFP='C:\Program Files\Google\Chrome\Application\chrome.exe'   # -mFP == microsofWIndows_FP !  !!due to space in it, the call MUST be enclosed as: "$chrom1_mFP" !
viv1_mFP="C:\Progs2\Vivaldi\Application\vivaldi.exe"
...
alias  chrom1='"$chrom1_mFP"  --incognito  > /dev/null  2>&1  &'
alias  viv1='"$viv1_mFP"      --incognito  > /dev/null  2>&1  &'
...
#####  ==========  OSs-addies/adapts/...:
#-??req?:  HOME="$(cygpath $HOME)"
winpath1() { echo "pathes-mswin+unix of Param1 and cu-dir:" ; cygpath --windows $PWD "$@" ; cygpath  --unix  $PWD  "$@" ; }   ##--Windows-unix-Pathes-of-PWD + $1
unalias psg ; alias  psg='ps -alW | grepi'   ##--I- ps cmd of MSYS2 ! #-/OR-extra-alias-for-msw:  alias psgw="ps -W | grepi";
alias  vipy='PATH="/bin/:$PATH"  vi1'   #-msys-vim requires msys-python-exe !
bind -f ${INPUTRC_mswin1}   ##--eg case-insensitive-path-completion,...; BUT eg for WLS/ms-Lx/.. may NOT!! so if other ms-Lx.systems in use, then swit to if .... as below/out-commented!
...
============================ WSL-/CYG-env , if required: ==================================
#####  ==========  WSL or CYG/MSYS is the cu-system ??:  ##--> for now both not active/obsol !
q_mswCyg=0 ; q_mswWsl=0
[[ $q_uname1infs =~ "MICROSOFT-STANDARD-WSL" ]] && q_mswWsl=1 || q_mswCyg=1  #--> but for now they are not used! no WSL-/CYG-env now!
###________________________________________  ___________________________


#####  ==========  CYG-systems (msys/cyg/...), so NOT-WSL-systems : ##########################
#--nts: cygwin-env is really obsolete after working with MSYS2! so just in case ... :
[[ $q_mswCyg > 0 ]] && {
	...
	# ##--1ok-mswinVimInstall:   alias vim="$vim_cmd1"
...
[[ $q_mswWsl > 0 ]] && {
	q_mswWsl=1
	winpath1() { echo "pathes-mswin+unix of Param1 and cu-dir:" ; cygpath --windows $PWD "$@" ; cygpath  --unix  $PWD  "$@" ; }   ##--Windows-unix-Pathes-of-PWD + $1
	##--1ok-mswinVimInstall:  alias  vi1="$vimMSsExeFP"  gvi1="$gvimMSsExeFP"  ##--the vim-envvars are correctlxy interpreted!
	##--if-required/org:   q_Cmp="/mnt/c"  ##-- C:-mountpoint in msys/cyg/...
}
##________________________________________  ___________________________

====================__1END__ WSL-/CYG-env , if required: ==================================
